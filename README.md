...
For repeatable local binding, copy `tools-dev.config.example.json` to `tools-dev.config.json` and set local host/port/origin values there. The local config file is gitignored; `--config <path>` switches profiles.
